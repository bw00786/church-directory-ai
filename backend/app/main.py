...
async def lifespan(app: FastAPI):
    ...
    if settings.slide_verify_enabled:
        try:
            ...
            await stop_slide_verification()
        except Exception:
            logger.exception("Failed to stop slide verification")
    try:
        from app.mixer.service import mixer_service
...
async def health_ready():
    ...
    # Readiness does not call the Anthropic API; /health/anthropic does.
    return {
        "status": "ready",
        "checks": {
            "api": "ok",
...
async def health_anthropic():
    """On-demand Claude inference check, not just configuration."""
